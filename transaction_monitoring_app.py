...
def apply_rules(row):
    ...
    if any(row.get(f"deposit_amount_T-{i}", 0) > 300000 for i in range(12)):
        reasons.append("Rule 4")
    if any(row.get(f"withdrawal_amount_T-{i}", 0) > 300000 for i in range(12)):
        reasons.append("Rule 5")
    if row.get("qow", 0) > 0 and row.get("Cash Transferred in SGD", 0) > 0.6 * row.get("qow"):
...
st.write("T0: First Month, T1: Second Month, T2: Third Month")
